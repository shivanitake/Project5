...
class UserService extends BaseService {
    ...
    add(user, callback, ctx) {
        ...
        var sql = "INSERT INTO st_user (FIRST_NAME,LAST_NAME,LOGIN,PASSWORD,DOB,MOBILE_NO,ROLE_ID,ROLE,GENDER,PIC,PIC_TYPE,PIC_NAME,CREATED_DATETIME,MODIFIED_DATETIME) "
            + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())";
        var params = [user.firstName, user.lastName, user.login,
        user.password, user.dob, user.mobileNo, user.roleId, user.role, user.gender, user.pic,user.picType,user.picName];
        super.executeSQL(sql, params, function (err, result) {
            if (err) {
                ...
                roleService.findByPk(user.roleId, function (rerr, role) {
                    if (!rerr) {
                        var upateSql = "UPDATE st_user SET ROLE=? WHERE ID = ?";
                        var params = [role.name, pk];
                        roleService.executeSQL(upateSql, params, function (rerr, sResult) {
                            callback(err, pk);
                        });
                    }
                }, ctx);
    ...
    update(user, callback, ctx) {
        var sql = "UPDATE st_user SET MODIFIED_DATETIME = NOW(), FIRST_NAME=?,LAST_NAME=?,LOGIN=?,PASSWORD=?,DOB=?,MOBILE_NO=?,ROLE_ID=?,ROLE=?,GENDER=? WHERE ID=?"
        var params = [user.firstName, user.lastName, user.login,
        user.password, user.dob, user.mobileNo, user.roleId, user.role, user.gender, user.id];
        super.executeSQL(sql, params, function (err, result) {
            if (err) {
                callback(err);
            } else {
                ...
                roleService.findByPk(user.roleId, function (rErr, role) {
                    if (!rErr) {
                        var updateSql = "UPDATE st_user SET ROLE=? WHERE ID = ?";
                        var params = [role.name, user.id];
                        roleService.executeSQL(updateSql, params, function (rErr, sResult) {
