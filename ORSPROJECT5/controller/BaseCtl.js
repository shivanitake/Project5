...
class BaseCtl {
    ...
    getdisplayRollNo(req, res) {
        var service = this.getService();
        var id = req.params.id;

        console.log("ritika"+id);
        service.findByRollNo(id, function (err, bean) {

            console.log("FIND BY ROLL NO CALL"+id);
    ...
    getMeritList(req, res) {
        ...
        service.getMeritList(bean, req.body.pageNo, function (err, result) {
            result.pageNo = pageNo;
            var r = new Response(err, result);
            res.json(r);
        });
